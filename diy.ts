...
game.import("character", function () {
	return {
		...
		character: {
			noname: ["female", "key", 9, ["noname_zhuyuan", "noname_duocai"]],
				...
				content() {
					...
					if (result.bool) {
						...
						if (result.links.length > 2) {
							...
							if (result.links.length == 2) player.draw(4);
		...
		translate: {
			...
			noname_duocai: "多彩",
			noname_duocai_info:
				"每回合限四次。其他角色区域内的牌因弃置而进入弃牌堆后，你可以获得之。若你以此法得到的牌数：大于2，你弃置一名角色区域内的一张牌；等于2，你摸一张牌；小于2，你回复1点体力。",
